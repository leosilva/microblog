...
from app.forms.main.edit_profile_form import EditProfileForm
from app.forms.main.empty_form import EmptyForm
...
def user(username):
    ...
    form = EmptyForm()
    return render_template('main/user.html', user=user, posts=posts.items, form=form,
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            flash(_('User {username} not found.'))
            return redirect(url_for('main.index'))
